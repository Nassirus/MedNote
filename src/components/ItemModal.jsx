...
import { canToggleItem } from '../lib/dateUtils'
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              {canToggleItem(item) ? (
                <button className="btn" onClick={() => { onToggle(item.id); onClose() }}
                  style={{ flex: 2, background: item.done ? 'var(--surface2)' : 'var(--success)', color: item.done ? 'var(--text2)' : 'white' }}>
                  {item.done ? '↩ Снять отметку' : '✓ Выполнено'}
                </button>
              ) : (
                <div style={{ flex: 2, padding: '11px', background: '#F8FAFC', border: '1px solid var(--border)',
