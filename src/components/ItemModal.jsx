        ...
        {editing ? (
          ...
          <>
            ...
            <div style={{ padding:'9px 14px', borderRadius:10, marginBottom:14, textAlign:'center',
              ...
              <span style={{ fontSize:13, fontWeight:600, color:done?'var(--success)':canToggle?'var(--warning)':'var(--text3)' }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                ...
                {canToggle ? (
                  <button className="btn" onClick={() => { onToggle(item.id); onClose() }}
                    style={{ flex:3, background:done?'var(--surface2)':'var(--success)', color:done?'var(--text2)':'white' }}>
                    {done ? '↩ Снять отметку' : '✓ Выполнено'}
                  </button>
                ) : (
                  <div style={{ flex:3, padding:'10px', background:'var(--surface2)', border:'1px solid var(--border)',
                    borderRadius:9, textAlign:'center', fontSize:13, color:'var(--text3)', fontWeight:500 }}>
