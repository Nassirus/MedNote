...
import { isConnected, addEventToGCal, getStoredToken } from '../lib/googleCalendar'
import {
...
export function useSchedule() {
  ...
  async function add(item) {
    ...
    try {
      ...
      if (getStoredToken()) {
        addEventToGCal({ ...cleanItem, endTime: end }).catch(() => {
          // Sync failure is non-blocking — item is already saved in Firestore
        })
      }
