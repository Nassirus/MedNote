...
function isItemToday(item) {
  if (!item.date) return true // recurring
  try {
    const d = typeof item.date === 'string' ? parseISO(item.date) : new Date(item.date.seconds * 1000)
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'var(--bg)' }}>
      <div className="page-header">
        ...
        <button className="btn btn-primary" onClick={() => setShowAdd(true)} style={{ padding:'8px 16px', fontSize:13 }}>
          + Добавить
        </button>
      </div>
      ...
      <div className="page-content">
        {dbError === 'index' && (
          <div style={{ background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:10, padding:'11px 14px', marginBottom:16, fontSize:12, color:'#92400E', lineHeight:1.6 }}>
            ⚠️ Нужен индекс Firestore: <strong>Firebase Console → Firestore → Indexes → Add index</strong><br/>
        ...
        <div className="hero-banner" style={{ marginBottom:18 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
            <div>
              <div style={{ fontSize:13, color:'rgba(255,255,255,0.75)', marginBottom:4 }}>Выполнение сегодня</div>
              <div style={{ fontSize:40, fontWeight:800, lineHeight:1, marginBottom:4 }}>
                {pct}<span style={{ fontSize:22 }}>%</span>
              </div>
              <div style={{ fontSize:13, color:'rgba(255,255,255,0.75)' }}>{done} из {total} пунктов</div>
            </div>
          ...
          {/* Progress bar */}
          <div style={{ height:6, background:'rgba(255,255,255,0.25)', borderRadius:99, marginTop:14, overflow:'hidden' }}>
            <div style={{ height:6, width:pct+'%', background:'white', borderRadius:99, transition:'width .5s ease' }} />
          </div>
          {/* Next up */}
          {upcoming && (
            <div style={{ marginTop:12, padding:'8px 12px', background:'rgba(255,255,255,0.15)', borderRadius:8, display:'flex', alignItems:'center', gap:8 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                ...
                <span style={{ fontSize:12, fontWeight:600, color:'white' }}>{upcoming.title}</span>
                <span style={{ fontSize:12, color:'rgba(255,255,255,0.7)' }}> в {upcoming.time}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {filtered.length === 0 && (
            <div className="empty-state">
              <div style={{ fontSize:44, marginBottom:12 }}>{total===0?'📋':filter==='done'&&done===0?'⏳':'🎉'}</div>
          ...
          {filtered.map(item => {
            ...
            return (
              <div key={item.id} style={{
                ...
                border:`1px solid ${item.done ? 'var(--border)' : ic.hex+'33'}`,
                borderLeft:`4px solid ${item.done ? 'var(--border)' : ic.hex}`,
                opacity: item.done ? 0.6 : 1, transition:'all .2s',
                ...
                <div style={{ flex:1, minWidth:0, cursor:'pointer' }} onClick={() => setSelected(item)}>
                  ...
                  <div style={{ display:'flex', gap:8, marginTop:3, alignItems:'center', flexWrap:'wrap' }}>
                    ...
                    <span style={{ padding:'2px 8px', borderRadius:99, background:ic.light, color:ic.hex,
                      fontSize:10, fontWeight:600 }}>{cfg.label}</span>
                    {item.notes && (
                      <span style={{ fontSize:11, color:'var(--text3)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:120 }}>
                        {item.notes}
                      </span>
                    )}
                  </div>
                ...
                <button onClick={() => toggle(item.id)} style={{
                  width:28, height:28, borderRadius:'50%', border:`2.5px solid ${item.done ? '#059669' : ic.hex}`,
                  background: item.done ? '#059669' : 'white', flexShrink:0,
                  display:'flex', alignItems:'center', justifyContent:'center',
                  boxShadow: item.done ? '0 0 0 3px #D1FAE5' : 'none', transition:'all .2s'
                }}>
      ...
      {selected && <ItemModal item={selected} onClose={() => setSelected(null)} onDelete={remove} onToggle={toggle} onUpdate={update} />}
    </div>
  )
}
