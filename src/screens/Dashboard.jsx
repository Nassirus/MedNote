...
export default function Dashboard({ items, toggle, add, remove, update }) {
  ...
  const filtered = filter === 'all' ? sorted : filter === 'done' ? sorted.filter(i => i.done) : sorted.filter(i => !i.done)
        ...
        <div className="card" style={{ padding: '18px 20px', marginBottom: 18 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 14 }}>
            <div>
              ...
              <div style={{ fontSize: 28, fontWeight: 800, color: pct === 100 ? 'var(--success)' : 'var(--primary)', lineHeight: 1.2 }}>{pct}%</div>
              <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{done} из {items.length} пунктов</div>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              {byType.slice(0, 3).map(([type]) => {
                const cfg = TYPE_CONFIG[type]
                return <div key={type} style={{ width: 34, height: 34, borderRadius: 9, background: cfg.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 17 }}>{cfg.icon}</div>
              })}
            </div>
          </div>
          <div style={{ height: 8, background: 'var(--surface2)', borderRadius: 999, overflow: 'hidden' }}>
            <div style={{ height: 8, width: pct + '%', background: pct === 100 ? 'var(--success)' : 'linear-gradient(90deg,var(--primary),#0D9488)', borderRadius: 999, transition: 'width .5s ease' }} />
          </div>
        </div>

        {/* Filter tabs */}
        <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
          {[['all', `Все (${items.length})`], ['pending', `Ожидает (${items.length - done})`], ['done', `Готово (${done})`]].map(([k, l]) => (
            <button key={k} onClick={() => setFilter(k)} style={{
              ...
              boxShadow: filter === k ? 'none' : 'var(--shadow)',
              transition: 'all .15s'
            }}>{l}</button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)' }}>
              ...
              <div style={{ fontWeight: 600, fontSize: 14 }}>Всё выполнено!</div>
            </div>
          )}
          {filtered.map(item => {
            ...
            return (
              <div key={item.id} className={`schedule-item${item.done ? ' done-item' : ''}`}>
                ...
                <div style={{ flex: 1, minWidth: 0, cursor: 'pointer' }} onClick={() => setSelected(item)}>
                  <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--text)', textDecoration: item.done ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</div>
                  <div style={{ display: 'flex', gap: 8, marginTop: 3, flexWrap: 'wrap' }}>
                    ...
                    {item.freq && item.freq !== 'Ежедневно' && <span style={{ fontSize: 11, color: 'var(--text3)' }}>{item.freq}</span>}
                  </div>
                </div>
