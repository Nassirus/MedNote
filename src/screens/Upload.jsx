...
export default function Upload({ onAddItems }) {
  ...
  async function handleFile(e) {
    ...
    // Treat as image if it's an image type OR if file extension is jpg/png/webp/heic
    ...
    if (isImage) {
      // Use readAsArrayBuffer → convert to base64 manually
      // This avoids "string did not match expected pattern" on HEIC and some JPEGs
      ...
      // Determine mimeType: if browser gave us empty/wrong type, guess from extension
      let mimeType = f.type
      if (!mimeType || mimeType === 'application/octet-stream') {
        const mimeMap = { jpg:'image/jpeg', jpeg:'image/jpeg', png:'image/png',
          webp:'image/webp', heic:'image/jpeg', heif:'image/jpeg', bmp:'image/bmp', gif:'image/gif' }
        mimeType = mimeMap[ext] || 'image/jpeg'
      }
      // For preview, use jpeg since HEIC can't be shown in browser
      const previewUrl = `data:${mimeType};base64,${base64}`
      setFile({ name: f.name, base64, mimeType, preview: previewUrl, isImage: true })
      setMode('photo')
    } else {
          ...
          <>
            {file?.preview ? (
              <div style={{ marginBottom:14, position:'relative' }}>
                ...
                <div style={{ marginTop:6, fontSize:12, color:'var(--text3)', textAlign:'center' }}>📎 {file.name}</div>
              </div>
            ) : (
