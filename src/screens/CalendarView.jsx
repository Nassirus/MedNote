          ...
          >
            ...
            <div style={{ minWidth: 420, position: 'relative', height: 24 * SLOT_H }}>
              ...
              {weekDays.map((day, dayIdx) => {
                ...
                const colWidth = `calc((100% - ${LABEL_W}px) / 7 - 2px)`
                return layouts.map((lev, i) => (
                  <div key={`${day.toISOString()}-${i}`} style={{
                    position: 'absolute',
                    top: (lev.start / 60) * SLOT_H,
                    height: Math.max(((lev.end - lev.start) / 60) * SLOT_H, 20),
                    left: `calc(${LABEL_W}px + ${dayIdx} * (100% - ${LABEL_W}px) / 7 + ${lev.colIdx} * (100% - ${LABEL_W}px) / 7 / ${lev.colCount})`,
                    width: `calc((100% - ${LABEL_W}px) / 7 / ${lev.colCount} - 2px)`,
                    zIndex: 3, pointerEvents: 'all',
                  }}
                    onMouseDown={e => e.stopPropagation()}
                    onClick={e => { e.stopPropagation(); setSelItem(lev.item) }}
                  >
