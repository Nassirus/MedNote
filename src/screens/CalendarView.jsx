...
// dayIdx: which day column (0-6), labelW: px width of time label column
function WeekEventBlock({ layoutEv, dayIdx, labelW, onOpen }) {
  ...
  const height = Math.max(((end - start) / 60) * SLOT_H, 20)
  // Each day column occupies (100% - labelW) / 7 of total width.
  // Within that column, events are split by colCount.
  const colW   = `calc((100% - ${labelW}px) / 7)`
  const left   = `calc(${labelW}px + ${dayIdx} * (100% - ${labelW}px) / 7 + ${colIdx} * (100% - ${labelW}px) / 7 / ${colCount})`
  const width  = `calc((100% - ${labelW}px) / 7 / ${colCount} - 2px)`
...
export default function CalendarView({ items, add, toggle, remove, removeGroup, update, allItems }) {
  ...
  const longPressTimer        = useRef(null)   // long-press for touch
  ...
  useEffect(() => {
    const onKey = e => {
      if (e.key==='Escape') {
        ...
        setDrag(null); isDown.current=false; dragRef.current=null
      }
    }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', background:'var(--bg)' }}>
      ...
      {viewMode === 'week' && (
        <div style={{
          ...
          {/* Day header row — fixed, no scroll */}
          <div style={{
            display: 'grid',
            ...
            <div style={{ borderRight: '1px solid var(--border)', minWidth: LABEL_W }} />
            {weekDays.map(day => (
              <div key={day.toISOString()} style={{
                ...
                {dayItems(day).length > 0 && (
                  <div style={{ fontSize: 9, fontWeight: 700,
                    color: isToday(day) ? 'var(--primary)' : 'var(--text3)', marginTop: 1 }}>
                    {dayItems(day).length}
                  </div>
                )}
              </div>
          ...
          <div style={{
            ...
            overflowX: 'auto',      /* horizontal scroll on narrow screens */
            position: 'relative',
            userSelect: 'none',
            background: 'white',
            ...
            onTouchStart={e => {
              ...
              // Start long-press timer — 400ms hold to begin drag
              longPressTimer.current = setTimeout(() => {
                startDrag(touchStartPos.current.x, touchStartPos.current.y)
              }, 400)
            }}
            onTouchMove={e => {
              const t = e.touches[0]
              if (isDown.current) {
                // Drag is active — prevent scroll and move drag
                e.preventDefault()
                moveDrag(t.clientX, t.clientY)
              } else {
                // Not dragging yet — check if finger moved (cancel long press)
          ...
          >
            {/* Inner wrapper: min-width for mobile horizontal scroll */}
            <div style={{ minWidth: 420, position: 'relative', height: 24 * SLOT_H }}>
              ...
              {HOURS.map(hour => (
                <div key={hour} style={{
                  ...
                  <div style={{
                    ...
                    padding: '3px 6px 0 0',
                    textAlign: 'right', fontSize: 10, color: 'var(--text3)', fontWeight: 500,
                  }}>
              ...
              {/* Clickable overlay for drag (above grid lines but below events) */}
              <div style={{
              ...
              {weekDays.map((day, dayIdx) => {
                ...
                // Each layoutEv is rendered by WeekEventBlock which handles
                // its own absolute position. We just pass the day column offset.
                return layouts.map((lev, i) => (
                  <WeekEventBlock
                    key={`${day.toISOString()}-${i}`}
                    layoutEv={lev}
                    dayIdx={dayIdx}
                    labelW={LABEL_W}
                    onOpen={it => setSelItem(it)}
                  />
                ))
              })}
              ...
              {dragOverlay && (
                <div style={{
                  position: 'absolute',
                  top: dragOverlay.top,
                  height: Math.max(dragOverlay.height, 14),
                  left: `calc(${LABEL_W}px + ${dragOverlay.dayIdx} * (100% - ${LABEL_W}px) / 7)`,
              ...
              {(() => {
                ...
                return (
                  <div style={{
                    position: 'absolute',
                    top, zIndex: 10, pointerEvents: 'none',
                    left: `calc(${LABEL_W}px + ${todayIdx} * (100% - ${LABEL_W}px) / 7)`,
                    width: `calc((100% - ${LABEL_W}px) / 7)`,
            ...
            </div>{/* inner minWidth wrapper */}
          </div>{/* scrollable time grid */}
        </div>
      )}
...
{showAdd && (
      ...
      {selItem && (
        <ItemModal item={selItem} onClose={() => setSelItem(null)}
          onDelete={remove} onDeleteGroup={removeGroup || (() => {})} onDeleteDates={() => {}} onToggle={toggle} onUpdate={update} allItems={allItems || items} />
      )}
    </div>
  )
}
