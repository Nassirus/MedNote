export default async function handler(req, res) {
  ...
  const MODELS = [
    'gemini-2.0-flash',          // #1: самая быстрая и стабильная
    'gemini-3-flash-preview',    // #2: новее, но иногда медленнее
    'gemini-2.0-flash-lite',     // #3: лёгкая версия, своя квота
  ]
  ...
  for (let i = 0; i < MODELS.length; i++) {
    ...
    const timeout = setTimeout(() => controller.abort(), Math.min(remaining - 500, 7000))
    ...
    try {
      ...
      if (e.name === 'AbortError') {
        ...
        console.log(`[${model}] Timeout после ${elapsed2}ms`)
        if (i < MODELS.length - 1 && 9500 - elapsed2 > 3000) {
          // Есть время — пробуем следующую модель
          console.log('Пробуем следующую модель...')
          continue
        }
        return res.status(504).json({
          error: 'AI не успел обработать фото. Попробуйте: 1) сделать фото светлее и чётче, или 2) скопировать текст выписки и вставить в текстовое поле.'
        })
      }
      throw e
    }
